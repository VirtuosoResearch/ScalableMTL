...
def eval_output(model, task_idx, train_loader, device, pretrain_state_dict, finetuned_state_dict, steps = 200):
    ...
    for batch in train_loader:
        ...
        pretrain_outputs = torch.log(probs/(1-probs))

        pretrain_gradients = pretrain_outputs.clone().detach() 
        for i in range(len(pretrain_outputs)):
            tmp_gradients = torch.autograd.grad(pretrain_outputs[i], parameters, retain_graph=True, create_graph=False)
            tmp_gradients = dict(
        ...
        probs = torch.nn.functional.softmax(logits[np.arange(batch_size), 0], dim = 1)
        probs = probs[np.arange(len(labels)), labels]
        finetuned_outputs = torch.log(probs/(1-probs)).detach()

        print(pretrain_outputs, pretrain_gradients, finetuned_outputs)
...
def main():
    ...
    original_norm = np.math.sqrt(compute_norm(model, state_dict))
    ...
    with open(f"./sampled_tasks/sample_{data_args.task_name}.txt", "r") as f:
        for line in f.readlines():
            ...
            for scale in np.arange(0.01, 0.11, 0.01): # np.arange(1, 11, 1)
                print("Perturbation scale: ", scale)
                total_diff = 0
                new_state_dict = generate_state_dict(model, state_dict, coef*scale*original_norm/np.linalg.norm(coef), device) 
                print("Norm:", compute_norm(model, state_dict, new_state_dict))
                for task_idx in task_idxes:
                    diff = eval_output(model, task_idx, task_to_train_dataloaders[task_idx], device, state_dict, new_state_dict)
                    total_diff += diff
