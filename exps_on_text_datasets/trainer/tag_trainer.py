...
class TAGTrainer(Seq2SeqTrainer):
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        train_sampler = self._get_train_sampler() if not hasattr(self.args, "train_multitask") else None
    ...
    def intialize_tag(self, task_to_train_datasets, collect_gradient_step = 10, update_lr = 1e-4, record_step = 10, affinity_dir = '', affinity_type='tag'):
        self.train_datasets = task_to_train_datasets
        self.train_data_loaders = {task: self.get_dataloader(train_dataset) for task, train_dataset in self.train_datasets.items()}
    ...
    def get_dataloader(self, train_dataset) -> DataLoader:
        """
        Returns the training :class:`~torch.utils.data.DataLoader`.
    ...
    def get_train_sampler(self, train_dataset) -> Optional[torch.utils.data.Sampler]:
    ...
    def compute_loss_and_gradients(self, data_loader, task_name, step=1):
        ...
        for _, inputs in enumerate(data_loader):
            ...
            count += 1
        loss = loss/count
        feature_gradients = torch.autograd.grad(loss, self.model.parameters(), retain_graph=False, create_graph=False,
                             allow_unused=True)
    ...
    def compute_tag_task_gains(self):
        ...
        for task in task_data_loaders.keys():
            ...
            other_tasks =  task_data_loaders.keys()
