...
def eval_output(model, train_loader, input_dim, task_idx, device, pretrain_state_dict, finetuned_state_dict):
    ...
    print(f"Average relative erro for task {task_idx}", diffs/counts)
    return diffs/counts
...
def compute_norm(model, pretrain_weights, finetuned_weights=None):
    ...
    for key, param in model.named_parameters():
        if finetuned_weights is None:
            norm += (torch.linalg.norm(pretrain_weights[key])).sum().cpu().item()
        else:
            norm += (torch.linalg.norm(finetuned_weights[key]-pretrain_weights[key])).sum().cpu().item()
    return norm
...
def test(model, data, task_idxes, device):
    log = {}
    model.eval()
    decoupling = True
    ...
    data = data.to("cpu")
    if decoupling:
        out = model.inference(data.xs, device = device, return_softmax = False)
    
    y_pred = (out > 0).type(torch.float)
    y_true = data.y
    ...
    for task_idx in task_idxes:
        ...
        train_losses[task_idx] = names_to_criterions["multilabel"](out[data.train_mask[:, task_idx]][:, task_idx], y_true[data.train_mask[:, task_idx]][:, task_idx]).cpu()
        valid_losses[task_idx] = names_to_criterions["multilabel"](out[data.val_mask[:, task_idx]][:, task_idx], y_true[data.val_mask[:, task_idx]][:, task_idx]).cpu()
        test_losses[task_idx]  = names_to_criterions["multilabel"](out[data.test_mask[:, task_idx]][:, task_idx], y_true[data.test_mask[:, task_idx]][:, task_idx]).cpu()
    # print(test_accs)
    log = {
            "train_loss": train_losses[task_idxes].mean().item(), 
            "valid_loss": valid_losses[task_idxes].mean().item(), 
            "test_loss": test_losses[task_idxes].mean().item(), 
            f"train_f1_score": train_accs[task_idxes].mean().item(), 
            f"valid_f1_score": valid_accs[task_idxes].mean().item(), 
            f"test_f1_score": test_accs[task_idxes].mean().item(), 
            f"valid_longtail_f1_score": 0, 
            f"test_longtail_f1_score": 0,
            f"train_precision": train_precisions[task_idxes].mean().item(), 
            f"valid_precision": valid_precisions[task_idxes].mean().item(), 
            f"test_precision":   test_precisions[task_idxes].mean().item(), 
        }
    for i, task_idx in enumerate(task_idxes):
        task_log = {
            f"task_{task_idx}_train_f1_score": train_accs[task_idx].item(), 
            f"task_{task_idx}_valid_f1_score": valid_accs[task_idx].item(), 
            f"task_{task_idx}_test_f1_score": test_accs[task_idx].item(), 
            f"task_{task_idx}_valid_longtail_f1_score": 0, 
            f"task_{task_idx}_test_longtail_f1_score": 0,
            f"task_{task_idx}_train_precision": train_precisions[task_idx].item(), 
            f"task_{task_idx}_valid_precision": valid_precisions[task_idx].item(), 
            f"task_{task_idx}_test_precision":   test_precisions[task_idx].item(), 
        }
        log.update(**task_log)
...
def main(args):
    ...
    # load checkpoint 
    checkpoint_dir = f"./saved/{args.dataset}_sign_3_256_all_run_{args.run}/model_best.pth"
    state_dict = torch.load(checkpoint_dir, map_location=device)
    model.load_state_dict(state_dict)


    # collect gradients
    gradients_dir = f"./gradients/{args.dataset}_{args.project_dim}/run_{args.run}"
    assert os.path.exists(gradients_dir)
        
    matrix_P = np.load(f"./gradients/{args.dataset}_{args.project_dim}/projection_matrix_{int(args.run%10)}.npy")

    subset_count = 0
    with open(f"./sampled_tasks/sample_{args.dataset}_128_10.txt", "r") as f:
        for line in f.readlines():
            ...
            '''save results'''
            log = test(model, data, task_idxes, device)
            ...
            for task_idx in task_idxes:
                ...
                for key, val in log.items():
                    if f"task_{task_idx}" in key:
                        ...
                        result_datapoint[metric_name] = val
            ...
            subset_count += 1
            if subset_count > args.num_subsets:
                break
    end = time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_subsets", type=int, default=2500)
